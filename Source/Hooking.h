...
typedef void(__cdecl* SetOPNetworkWeather)(int eventGroup, int weatherID, int a3, _int64 alwaysZero);
typedef char* (__cdecl* fpGetPlayerName)(Player player);
typedef bool(__fastcall* fpCNetworkIncrementStatEvent)(uint64_t net_event_struct, int64_t sender, int64_t a3);
...
typedef BOOL(__cdecl* fpGetWaypointPos)(Vector3* pos);
typedef BOOL(__cdecl* fpNetworkIsSessionStarted)();
typedef int(__cdecl* Trigger_Script_Event__)(int unk0, uint64_t* args, int argCount, int bitFlags);
...
public:
	...
	static fpCNetworkIncrementStatEvent              m_CNetworkIncrementStatEventt;
	static SetOPNetworkWeather                       set_network_weather;
	...
	static Trigger_Script_Event__        	         trigger_script_event;

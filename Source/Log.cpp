...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: Arctic");
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			...
			OutputDebugStringA("INIT: Arctic");
		}
		else {
			useFileOutput = false;
		}
...
void Log::Msg(const char* fmt, ...) {
	...
	if (useFileOutput) {
		FILE* file;
		if ((fopen_s(&file, g_debugLogFile, "a")) == 0) { fprintf_s(file, "%s", chLogBuff); fclose(file); } std::cout << chLogBuff;
	}
	sprintf_s(chLogBuff, "%s\n", chParameters);
	OutputDebugStringA(chLogBuff);
}

...
void ImgNotify(char* text, char* text2, char* Subject)
{
	UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
	UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(text);
	UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG("Logo", "Logo", false, 7, text2, Subject, 1.0, "Shadow");
	UI::_DRAW_NOTIFICATION(FALSE, FALSE);
}
...
//{
	...
	//If statement and while loops
		...
		//{
			...
			//notifyleft2("Welcome To Shadow Menu", "Shadow", "V-1.0.0"); //Message, title, subtitle - notifyleft2
			//notifyleft2("Setting up menu! Wait...", "Shadow", "SETUP"); //Message, title, subtitle - notifyleft2
			...
			//notifyleft2("Open menu with F8", "Shadow", "CONTROLS"); //Message, title, subtitle - notifyleft2
			//NULL;
		//}
...
//
		...
		//{
			...
			//Disable open button if username and key is wrong
				...
				//notifyleft2("Username or Key was wrong! GTA will be exited in 5 sec...", "Shadow", "~r~ERROR:"); //Message, title, subtitle - notifyleft2
				//notifyleft2("", "Shadow", "Purchase at:"); //Message, title, subtitle - notifyleft2
...
void WriteHotkey(std::string name) 
{
	...
	std::string FullPath = safePath + ".C:\\Arctic\\HotKeys\\keys.ini";
	...
	INI::WriteInt(FullPath, key, "hotkeys", name);
}
	...
	if (NETWORK::NETWORK_IS_SESSION_ACTIVE())
	{
		...
		for (int i = 0; i < 32; i++)
		{
			...
			for (auto id : Admins)
			{
				if (UserID = id)
				{
					notifyMap("R* admin found in session, it's recommended you leave the session immediately!");
				}
			}
		}
	}
...
void main() {
	...
	//		notifyleft2("Incorrect HWID", "FELXY", "~r~ERROR");
	...
	//		notifyleft2("Try again in 10 sec...", "FELXY", "~r~ERROR");
	//		CONTROLS::DISABLE_CONTROL_ACTION(VK_F8, VK_F8, true);
	//		WAIT(10000);
	//		main();
	//	}
	...
	//	std::wstring hwidfinallink = (L"" + s2ws(username) + L"&password=" + s2ws(password) + L"&hwid=" + s2ws(combinedHwid));
	...
	//		//notifyleft2("Welcome to Shadow. Open the menu with F8", "Shadow", "STANDARD");

	//		//Drawing Welcome Message
	//		char WelcomeText[100];
	//		sprintf(WelcomeText, "Welcome to ~g~Shadow MENU~w~ %s", Username);
	//		ImgNotify(WelcomeText, "", "STANDARD");
	//		ImgNotify("Open the menu with F8", "Shadow", "CONTROLS");
	//	}
	//	else
	//	{
	//		Log::Msg("Incorrect Login");
	//		notifyleft2("Wrong username or password", "FELXY", "~r~ERROR");
	//		notifyleft2("Try again in 3 sec...", "FELXY", "~r~ERROR");
	//		CONTROLS::DISABLE_CONTROL_ACTION(VK_F8, VK_F8, true);
	//		WAIT(3000);
	//		main();
	//	}
	...
	while (true) {
		//Inluding all void in menu - Functions!
		...
		case mainmenu:
		{
			...
			Menu::MenuOption("Self Options", sub);
			Menu::MenuOption("Weapon Menu", weps);
			Menu::MenuOption("Vehicle Menu", vehoptions);
			Menu::MenuOption("Teleport Menu", new_tele);
			Menu::MenuOption("World Options", world_options);
			...
			{
				Menu::OptionInfo("Take control over online players...");
			}
...
#pragma region Self Menu
		case sub:
		{
			...
			//Menu::Toggle("Big Player", big_player);
			//To use functions do _>
			Menu::MenuBreak("[Merryweather - Online Only]");
			if (Menu::Option("Airstrike")) //maybe to all 
			{
				globalHandle(2531497).At(4390).As<int>() = 4390;
			}
			if (Menu::Option("Bullshark"))
			{
				globalHandle(2531497).At(841).As<int>() = 841;
			}

			if (Menu::Option("Ammo Drop"))
			{
				globalHandle(2531497).At(833).As<int>() = 833;
			}
			if (Menu::Option("Heli Backup"))
			{
				globalHandle(2531497).At(4389).As<int>() = 4389;
			}
		}
		break;
#pragma endregion
...
#pragma region recovery
		case recovery:
		{
			...
			Menu::MenuBreak("[Money Options]");
			Menu::Toggle("Circle Drop", cyrclenew);
			Menu::Toggle("Money Drop", moneyDrop);
			if (Menu::Option("15 Million Stealth"))
			{
				int val = -1586170317;
				globalHandle(4264315 + 1).As<int>() = rndInt(0, INT_MAX - 100000);
				globalHandle(4264315 + 6).As<int>() = 0;
				globalHandle(4264315 + 5).As<int>() = 0;
				globalHandle(4264315 + 3).As<int>() = val;
				globalHandle(4264315 + 2).As<int>() = 15000000;
				globalHandle(4264315).As<int>() = 1;
			}
			if (Menu::Option("10 Million Stealth")) // NOTE DONT SHARE - Will never
			{
				int val2 = -1586170317;
				globalHandle(4264315 + 1).As<int>() = rndInt(0, INT_MAX - 100000);
				globalHandle(4264315 + 6).As<int>() = 0;
				globalHandle(4264315 + 5).As<int>() = 0;
				globalHandle(4264315 + 3).As<int>() = val2;
				globalHandle(4264315 + 2).As<int>() = 10000000;
				globalHandle(4264315).As<int>() = 1;
			}
			if (Menu::Option("5 Million Stealth"))
			{
				int val2 = -1586170317;
				globalHandle(4264315 + 1).As<int>() = rndInt(0, INT_MAX - 100000);
				globalHandle(4264315 + 6).As<int>() = 0;
				globalHandle(4264315 + 5).As<int>() = 0;
				globalHandle(4264315 + 3).As<int>() = val2;
				globalHandle(4264315 + 2).As<int>() = 5000000;
				globalHandle(4264315).As<int>() = 1;
			}
			if (Menu::Option("10K Drop"))
			{
				spawn_money_safe(PLAYER::PLAYER_PED_ID());
			}
			if (Menu::Option("30K Drop"))
			{
				spawn_money_safee(PLAYER::PLAYER_PED_ID());
			}
			Menu::MenuBreak("[Outfits]");
			if (Menu::Option("Random Outfit")) //wORKING
			{
				PED::R_C_T_P(PLAYER::PLAYER_PED_ID(), true);  // where is off radar ? Not here it wasent working so i commented it out and then sadly deleted it // can i check ur fastrun ? Yhe it dosent work  :facepalm:
			}
			Menu::MenuOption("Outfit Editor", out_create);
			//Menu::MenuOption("Men Outfits", men_outf);
			//Menu::MenuOption("Women Outfits", women_outf);
			Menu::MenuBreak("[Unlocks]");
			if (Menu::Option("Full Account Recovery"))
			{
				Recovery::FullRecovery();//Let's see
			}
...
#pragma region New Teleport
		case new_tele:
		{
			...
			Menu::MenuBreak("Quick Teleports");
...
#pragma region Special Options
		case special_options:
		{
			...
			{
				notifyleft2("You are now in cop mode!", "Shadow", "Cop mode ~g~Enabled!");
...
#pragma region vehspawnerr
		case vehspawner:
		{
			...
			if (Menu::Settings::currentOption == 1)
			{
				Menu::OptionInfo("Spawn any sport vehicle");
			}
			if (Menu::Settings::currentOption == 2)
			{
				Menu::OptionInfo("Spawn any super vehicle");
			}
			if (Menu::Settings::currentOption == 3)
			{
				Menu::OptionInfo("Spawn any coupes vehicle");
			}
			if (Menu::Settings::currentOption == 4)
			{
				Menu::OptionInfo("Spawn any arena wars vehicle");
			}
			if (Menu::Settings::currentOption == 5)
			{
				Menu::OptionInfo("Spawn any casino vehicle");
			}
			if (Menu::Settings::currentOption == 6)
			{
				Menu::OptionInfo("Spawn any bike vehicle");
			}
			if (Menu::Settings::currentOption == 7)
			{
				Menu::OptionInfo("Spawn any boat vehicle");
			}
			if (Menu::Settings::currentOption == 8)
			{
				Menu::OptionInfo("Spawn any plane...");
			}
			if (Menu::Settings::currentOption == 9)
			{
				Menu::OptionInfo("Spawn any helicopter...");
			}
			if (Menu::Settings::currentOption == 10)
			{
				Menu::OptionInfo("Spawn any modded vehicle...");
			}
...
#pragma region Portable LSC
		...
		break;
#pragma endregion
//Make all vehicle pages
#pragma region Casino
		case casino:
		{
			Menu::Title("Casino Heist");
			Menu::SmallTitle("Casino Vehicles");
			Menu::VehicleOption("Formula");
			Menu::VehicleOption("Formula2");
			Menu::VehicleOption("Furia");
			Menu::VehicleOption("Imorgon");
			Menu::VehicleOption("Kanjo");
			Menu::VehicleOption("Minitank");
			Menu::VehicleOption("Outlaw");
			Menu::VehicleOption("Rebla");
			Menu::VehicleOption("Retinue2");
			Menu::VehicleOption("Stryder");
			Menu::VehicleOption("Sugoi");
			Menu::VehicleOption("Vagrant");
			Menu::VehicleOption("Vstr");
			Menu::VehicleOption("Yosemite2");
			Menu::VehicleOption("Zhaba");
			//15 st all works! ONLINE
...
#pragma region arena
		...
		{
			Menu::Title("");//Use this line to include banner on option page
			Menu::SmallTitle("Arena War");//Use this line to include banner on option page
			char* aarena[] = 
			{ 
				"bruiser", 
				"bruiser2", 
			...
			Menu::Title("arena");
			for (int i = 0; i < ARRAYSIZE(aarena); i++)
			{

				Menu::VehicleOption(aarena[i]);
			}
         }
		break;
#pragma endregion
...
#pragma region Sports
		case sport:
		{
			...
			Menu::VehicleOption("kuruma2"); //1
            Menu::VehicleOption("kuruma"); //2
			Menu::VehicleOption("Tailgater"); //3
			Menu::VehicleOption("RapidGT"); //4
			Menu::VehicleOption("RapidGT2"); //5
			Menu::VehicleOption("Oracle"); //6
			Menu::VehicleOption("sultanrs"); //7
			Menu::VehicleOption("banshee2");//8
			Menu::VehicleOption("Jugular");//9
			Menu::VehicleOption("Schlagen"); //10
			Menu::VehicleOption("Omnis"); //11
			Menu::VehicleOption("sheava");//12
			Menu::VehicleOption("buffalo3");//13
			Menu::VehicleOption("elegy2");//14
			Menu::VehicleOption("comet2");//15
...
#pragma region Super Vehicle
		case super_cars:
		{
			Menu::Title("");
			Menu::Title("Super");
			Menu::VehicleOption("manana");
			Menu::VehicleOption("jb700");
			Menu::VehicleOption("tornado");
			Menu::VehicleOption("tornado2");
			Menu::VehicleOption("tornado3");
			Menu::VehicleOption("tornado4");
			Menu::VehicleOption("stinger");
			Menu::VehicleOption("stingergt");
			Menu::VehicleOption("casco");
			Menu::VehicleOption("pigalle");
			Menu::VehicleOption("monroe");
			Menu::VehicleOption("ztype");
			Menu::VehicleOption("chino");
			Menu::VehicleOption("peyote");
		}
		break;
#pragma endregion
...
#pragma region Bikes
		...
		{
			Menu::Title("");
			Menu::SmallTitle("Bikes");
			Menu::VehicleOption("akuma");
			Menu::VehicleOption("double");
			Menu::VehicleOption("thrust");
			Menu::VehicleOption("hexer");
			Menu::VehicleOption("sanchez2");
			Menu::VehicleOption("sanchez");
			Menu::VehicleOption("carbonrs");
			Menu::VehicleOption("bati");
			Menu::VehicleOption("bati2");
			Menu::VehicleOption("faggio2");
			Menu::VehicleOption("ruffian");
			Menu::VehicleOption("nemesis");
			Menu::VehicleOption("pcj");
			Menu::VehicleOption("vader");
			Menu::VehicleOption("bagger");
			Menu::VehicleOption("daemon");
		}
		break;
#pragma endregion
...
#pragma region Boats
		case Boats:
		{
			...
			Menu::VehicleOption("marquis");
			Menu::VehicleOption("dinghy");
			Menu::VehicleOption("dinghy2");
			Menu::VehicleOption("seashark");
			Menu::VehicleOption("seashark2");
			Menu::VehicleOption("jetmax");
			Menu::VehicleOption("squalo");
			Menu::VehicleOption("suntrap");
			Menu::VehicleOption("tropic");
			Menu::VehicleOption("submersible");
			Menu::VehicleOption("predator");
		}
		break;
#pragma endregion
...
#pragma region Planes
		case Planes:
		{
			...
			Menu::VehicleOption("blimp");
			Menu::VehicleOption("luxor");
			Menu::VehicleOption("shamal");
			Menu::VehicleOption("cuban800");
			Menu::VehicleOption("duster");
			Menu::VehicleOption("jet");
			Menu::VehicleOption("stunt");
			Menu::VehicleOption("mammatus");
			Menu::VehicleOption("lazer");
			Menu::VehicleOption("titan");
			Menu::VehicleOption("velum");
		}
		break;
#pragma endregion
...
#pragma region Helicopters
		case Helicopters_Car:
		{
			...
			Menu::VehicleOption("annihilator");
			Menu::VehicleOption("buzzard");
			Menu::VehicleOption("buzzard2");
			Menu::VehicleOption("cargobob");
			Menu::VehicleOption("cargobob2");
			Menu::VehicleOption("cargobob3");
			Menu::VehicleOption("frogger2");
			Menu::VehicleOption("polmav");
			Menu::VehicleOption("maverick");
			Menu::VehicleOption("skylift");
		}
		break;
#pragma endregion
//Normal pages again
#pragma region Session
		case session:
		{
			...
			Menu::MenuBreak("[Spoofer]");
			Menu::MenuOption("Change Name", cha_na);
			if (Menu::Option("Rockstar ID Spoof~r~[In Process]"))
			{
				notifyleft("This feature are in process...You'll find it in next update!");
			}
			if (Menu::Int("Spoof Level Amount", lvlvalue, 0, 8000))
			{
				if (IsKeyPressed(VK_NUMPAD5) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlFrontendAccept)) {
					rpvalue = NumberKeyboard();
				}
			}
			if (Menu::Option("Enable Spoofed Level"))
			{
				if (NETWORK::NETWORK_IS_IN_SESSION())
				{
					SpoofOnlineLevel(lvlvalue);
				}
				//If player is in storymode. Don't do funtion!
				else if (!NETWORK::NETWORK_IS_IN_SESSION())
				{
					const char* DisplayThis = "~r~ERROR: ~w~You need to be online to use this feature!";
					notifyMap(DisplayThis);
				}
			}
			...
			if (Menu::Settings::currentOption == 6)
			{
				Menu::OptionInfo("Spoof your rockstar id");
			}
			if (Menu::Settings::currentOption == 7)
			{
...
#pragma region Display Name
		case cha_na:
		{
			...
			Menu::MenuBreak("[Set Name With Color]");
...
#pragma region Online Vehicle
		case player_veh:
		{
			...
			if (Menu::Option("Make Players Vehicle Jump"))
			{
				...
				{
					notifyleft2("Player are not in any vehicle!", "Shadow", "Error - 0x026c532");
				}
			}
			...
			if (Menu::Option("Max Players Vehicle"))
			{
				if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::GET_PLAYER_PED(selectedPlayer), true))
				{
					...
					VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(Netvehicle, "Shadow");
			...
			Menu::MenuBreak("[Spawn Vehicle To Them]");
		...
		case player_weapon:
		{
			...
			if (Menu::Option("Give Player All Weapons"))
			{
				...
				for (int i = 0; i < (sizeof(Weapons) / 4); i++)
				    ...
				    notifyleft2("You gived all weapons!", "Shadow", PLAYER::GET_PLAYER_NAME(selectedPlayer));
			}
			if (Menu::Option("Give All MK2 Weapons"))
			{
				uint Weapons[] = { GAMEPLAY::GET_HASH_KEY("weapon_revolver_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_snspistol_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_smg_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_pumpshotgun_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_assaultrifle_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_carbinerifle_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_specialcarbine_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_bullpuprifle_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_combatmg_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_heavysniper_mk2"), GAMEPLAY::GET_HASH_KEY("weapon_marksmanrifle_mk2"), };
				for (int i = 0; i < (sizeof(Weapons) / 4); i++)
					hooks::GIVE_DELAYED_WEAPON_TO_PED(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(selectedPlayer), Weapons[i], 9999, 1);
				    notifyleft2("You gived all  MK2 weapons!", "Shadow", PLAYER::GET_PLAYER_NAME(selectedPlayer));
			}
			if (Menu::Option("Remove All Weapons"))
			{
				WEAPON::REMOVE_ALL_PED_WEAPONS(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(selectedPlayer), true);
				notifyleft2("You removed all weapons!", "Shadow", PLAYER::GET_PLAYER_NAME(selectedPlayer));
			}
			Menu::MenuBreak("[Weapon Mods]");
...
#pragma region Player Trolling
		case player_Troll:
		{
			...
			Menu::MenuOption("E X T R A   T R O L L I N G", EXTRA_trolling);
			Menu::MenuOption("R E M O T E   O P T I O N S", remote_player);
			...
			if (Menu::Option("Orange Big ball"))
			{
				Hash oball = GAMEPLAY::GET_HASH_KEY("prop_juicestand");
				hooks::REQUEST_MODELL(oball);
				while (!hooks::HAS_MODEL_LOADEDD(oball))
					WAIT(0);
...
#pragma region Remote Options
		case remote_player:
		{
			...
			Menu::MenuBreak("[Trolling Messages]");
			if (Menu::Option("Troll Message"))
			{
				NewOption::TrollMessage();
			}
...
#pragma region Player Objects
		case obj_player:
		{
			...
			if (Menu::Option("Big Orange Ball"))
			{
...
#pragma region Player Teleport Locations
		case player_tele_loc:
		{
			Features::GetPlayerInfo(PLAYER::GET_PLAYER_NAME(selectedPlayer), selectedPlayer);//This is drawing out information about player etc...
			Menu::Title("");
			Menu::SmallTitle("Teleport To Locations");
			if (Menu::Option("Eclipse Towers")) { Network::Teleport1(false); }
			if (Menu::Option("West Vinewood")) { Network::Teleport2(false); }
			if (Menu::Option("Las Lagunas")) { Network::Teleport3(false); }
			if (Menu::Option("Spanish Ave")) { Network::Teleport4(false); }
			if (Menu::Option("Power St")) { Network::Teleport5(false); }
			if (Menu::Option("Del Perro")) { Network::Teleport6(false); }
			if (Menu::Option("Milton Rd")) { Network::Teleport7(false); }
			if (Menu::Option("The Royale")) { Network::Teleport8(false); }
			if (Menu::Option("La Puerta")) { Network::Teleport9(false); }
			if (Menu::Option("Rockford Dr")) { Network::Teleport10(false); }
			if (Menu::Option("Dream Tower")) { Network::Teleport11(false); }
			if (Menu::Option("San Vitas")) { Network::Teleport12(false); }
			if (Menu::Option("Vespucci Blvd")) { Network::Teleport13(false); }
			if (Menu::Option("Cougar Ave")) { Network::Teleport14(false); }
			if (Menu::Option("Prosperity St")) { Network::Teleport15(false); }
			if (Menu::Option("Weazel Plaza")) { Network::Teleport16(false); }
			if (Menu::Option("Del Perro Office")) { Network::Teleport17(false); }
			if (Menu::Option("Maze Bank West Office")) { Network::Teleport18(false); }
			if (Menu::Option("Arcadius Office")) { Network::Teleport19(false); }
			if (Menu::Option("Maze Bank Office")) { Network::Teleport20(false); }
		}
		break;
#pragma endregion
...
#pragma region Online All Players
		case all_players:
		{
			...
			Menu::MenuOption("Teleports", allP_teleport);
			Menu::MenuOption("Drops", all_drops);
			//Menu::MenuOption("Waypoint Money", lobby_rain);
			Menu::Toggle("ESP All Players", esp_all);
			//Menu::Toggle("Money Drop", money_all);
...
#pragma region lobby message
		case ses_mgs:
		{
			...
			/*if (Menu::Option("Purchase at ~g~Shadows Discord Here"))
			{
				for (int i = 0; i < 33; i++)
				{
					char* mgs = "Purchase menu at ~g~Shadows Discord";
					int handle[76];
					NETWORK::N_H_F_P(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(i), &handle[0], 13);
...
#pragma region new protections
		case NewProtect:
		{
			...
			//Menu::Toggle("General 2", Protect::EasyGeneral, [] { Protect::GeneralHidden(Protect::EasyGeneral); });                    //5
...
#pragma region Misc
		case misc:
		{
			...
			//Menu::Toggle("EventLogger", Features::lelellelellelelelellelellelellele);
			//Menu::Toggle("Show unknown events", Features::lellelellelelellelelellelelelellellelellelele);
			...
			if (Menu::Option("Max NightClub Popularity")) 
			{
				...
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("Textures", "CHAR_BUGSTARA", 1, 8, "GuardCheats", "Stats", 1, "", 9, 0);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			...
			{
				notifyleft2("You are now in police mode!", "Shadow", "Cop mode ~g~Enabled!");
				Ped Thisped = PLAYER::GET_PLAYER_PED(PLAYER::PLAYER_ID());
				PED::SET_PED_AS_COP(Thisped, true);
			}
			...
			//Menu::Toggle("Cursor Mode", Cursor::cursorOn);
			if (Menu::Settings::currentOption == 1)
			{
			...
			if (Menu::Settings::currentOption == 12)
			{
				Menu::OptionInfo("Makes the menu support mouse clicks and moves");
			}
...
#pragma region Settings Menu
		case settings:
		{
			...
			Menu::MenuBreak("~y~Social Club");
			Menu::Option(PLAYER::GET_PLAYER_NAME(PLAYER::PLAYER_ID()));
			Menu::MenuBreak("~g~Load Into Prologue");
